...
Plugin-Version: **0.9.27** — synchronisiert Bücher aus Calibre mit der Tolino Cloud.
...
**Blob-Lesung im Live-Grab & ehrliche Fensterlebensprüfung (0.9.27):** Zwei Feldbefunde: (1) „Das Anmeldefenster wurde geschlossen", obwohl das Fenster offen und noch nicht angemeldet war — während der Anmeldung verlässt der Tab `mytolino.com` zugunsten der Keycloak-Anmeldeseite des Partners (`www.orellfuessli.ch/...`), und eine kurze DevTools-Antwortpause sah identisch aus. „Geschlossen" wird jetzt ausschließlich vom DevTools-Endpunkt selbst entschieden (mehrere Fehlschläge in Folge); solange der Endpunkt antwortet, zählt das Fenster als offen und das Plugin wartet mit einem Hinweis in der Statuszeile weiter. (2) „Kein frischer Token" trotz angemeldetem Fenster — das Grab-Snippet prüfte Speicher-Werte nur auf JWT-Form und parste die JSON-Blobs (`oidc.user:…`) des Readers nie: der aktuelle Token im Blob war damit für Initial-Grab **und** Storage-Polling unsichtbar, und die Poll-Antworten kamen als JSON-String an, den die `isinstance`-(dict)-Prüfung still verwarf (nur die Test-Server antworteten mit dict, deshalb fiel die Regression nicht auf). Beide Snippets parsten Blobs jetzt top-level, das Polling liest zusätzlich die IndexedDB-Datenbanken der Seite mit (gedrosselt), abgefangene Token-Antworten werden mit Original-Status/-Headern/-Body an den Reader durchgereicht (vorher ein leerer 200er bzw. ein Continue, der am Response-Stage hängen bleiben kann), und der Dialog zeigt die Fortschrittstexte des Live-Grabs live in der Statuszeile an.
